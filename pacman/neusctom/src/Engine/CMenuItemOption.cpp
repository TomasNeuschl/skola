...
CMenuItemOption::CMenuItemOption(std::string label, int id, unsigned int &asigned_value) : CMenuItem(label, id),
...
void CMenuItemOption::draw(CWindow *wind, int x, int y, int width, bool highlight)
{
    if (id == asigned_value)
    {
        ...
        if (highlight)
            wind->print_str(printsring, x, y, COLOR_PAIR(PAIR_RED_BLACK));
        else
            wind->print_str(printsring, x, y, COLOR_PAIR(PAIR_YELLOW_BLACK));
    }
    else
    {
        if (highlight)
            wind->print_str(label, x, y, COLOR_PAIR(PAIR_RED_BLACK));
        else
