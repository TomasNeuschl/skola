...
CGhost::CGhost(int x, int y) : CCharacter(x,y), frightened(false)
{
	coords.first = x;
	coords.second = y;
}
...
{
	window->print((int)'G', coords.first, coords.second, COLOR_PAIR(PAIR_YELLOW_BLACK));
}
void CGhost::draw(CWindow *window, int colour_pair)
{
	window->print((int)'F', coords.first, coords.second, COLOR_PAIR(colour_pair));
}
